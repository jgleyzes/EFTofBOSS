...
def lnlike(theta, xdata, ydata, Cinv, free_para, fix_para,bounds,Om_fid, binning=False,TableNkmu=None, window=True,dataQ=None,withBisp=False,masktriangle=None,Bispdata=None,withPlanck=False):
    ...
    # Import the power spectra interpolators on the grid
        ...
        if withPlanck:
            return -0.5* ( chi2 + (rd-rs(Om,h))**2/sigma_rd**2 )
        else:
        	return -0.5* chi2


def lnprob(theta, xdata, ydata, Cinv, free_para, fix_para,bounds,Om_fid,binning=False,TableNkmu=None, window=True,dataQ=None,withBisp=False,masktriangle=None,Bispdata=None,withPlanck=False):
    ...
    dummy  =  lp + lnlike(theta, xdata, ydata, Cinv, free_para, fix_para,bounds,Om_fid, binning=binning,TableNkmu=TableNkmu, window=window,dataQ=dataQ,withBisp=withBisp,masktriangle=masktriangle,Bispdata=Bispdata,withPlanck=withPlanck)
...
#############################
    ...
    #################################
        ...
        t0 = time.time()    
        chi2  =  lambda theta: -2 * lnlike(theta,xdata, ydata, Cinv, free_para, fix_para,bounds,Om_fid,binning=binning,window=window,withBisp=withBisp,dataQ=dataQ,TableNkmu=TableNkmu,Bispdata=Bispdata,masktriangle=masktriangle,withPlanck=withPlanck)
    ...
    for jj in range(0, Nchains):
        ...
        sampler.append(emcee.EnsembleSampler(nwalkers, ndim, lnprob,a = 1.15, args = (xdata, ydata, Cinv, free_para, fix_para,bounds,Om_fid),kwargs={'binning':binning,'window':window,'withBisp':withBisp,'dataQ':dataQ,'masktriangle':masktriangle,'TableNkmu':TableNkmu,'Bispdata':Bispdata,'withPlanck':withPlanck},threads = 4))
