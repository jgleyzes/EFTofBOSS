#!/usr/bin/env python
###############UNDER CONSTRUCTION########################
## To implement the marginalized bispectrum there are a few changes that must be made
## Especially, one has to implement correctly the 'A' and 'C' matrices in the Gaussian likelihood
## The C matrix extends trivially once everything else is correctly working for A
## A step-by-step procedure to fix this is:
## 1) Extend the vector lengths in get_Pi_for_marg() to have the correct dimensions of k+kbisp
## padded with zeros for the terms that appropriately don't have contributions
## 
## 2) Add a b11 term in get_Pi_for_marg() for the bispectrum, also padded with zeros at P(k)
## Note this comes with a b1 multiplication, so we must do like b1*Bisp()
##
## 3) Remove the marginalization over b8. Adding the bispectrum breaks it.
##
## 4) Re-introduce correctly the bispectrum dependence in the likelihood as a big theory vector
##    T = (P(k), Bisp)
##
## 5) Modify the window function code such that it takes large covariance matrices with a bispectrum
## This will be done by making it be block structured ((Window, 0), (0, Identity))
...
def get_Pi_for_marg(Ploop,b1, bisp=None):
    ...
    if withBisp:
        ...
        postpad = np.zeros(sum(masktriangle))
        prepad = np.zeros(Ploop.shape[0])
        Pi = np.array([np.concatenate((Ploop[:,3,:]+b1*Ploop[:,7,:], postpad)),
             np.concatenate((Ploop[:,15,:]+b1*Ploop[:,12,:], postpad)),
             np.concatenate((Ploop[:,16,:]+b1*Ploop[:,13,:], postpad)),
             np.concatenate((Ploop[:,17,:]+b1*Ploop[:,14,:], postpad)), 
             np.concatenate((Ploop[:,19,:], postpad)),
             np.concatenate((Ploop[:,20,:], postpad)),
             np.concatenate((prepad, b1*Bisp[:,4,:]))])
...
def lnlike(theta,  kpred,chi2data,Cinvwdata,Cinvww, free_para, fix_para,bounds,Om_fid, marg_gaussian=False,binning=False,TableNkmu=None):
    ...
    # Import the power spectra interpolators on the grid
        ...
        if not binning:
            ...
            if marg_gaussian:
                Pi_AP = APpowerspectraNkmu.changetoAPnobinningPi(Pi_or[:,:len(kfull)],kfull,kfullred,qperp,qpar,nbinsmu=100)
            ...
            if type(TableNkmu) == type(None):
                ...
                if marg_gaussian:
                    Pi_AP = APpowerspectraNkmu.changetoAPbinningPi(Pi_or[:,:len(kfull)],kfull,kfull,qperp,qpar,TableNkmu)
        ...
        if marg_gaussian:
            ...
            if withBisp:
                ...
                Pi_bisp_only = Pi_or[:,len(kfull)]
                Pi_tot = np.concatenate((Pi_extrap, Pi_bisp_only), axis=1)
	...
	if 'Challenge' in simtype:
        ...
        for indexk,kmax in enumerate(kmaxtab):    
            ...
            if 'Challenge' in simtype and 'Quarter' not in simtype:
                ...
                kpred,Cinvw,Cinvww = WindowFunctionFourier.apply_window_covariance(Cinv,xdata,thin=2, withBisp, indextriangle)
