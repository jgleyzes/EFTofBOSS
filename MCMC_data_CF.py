...
import getcorrelationfunctionwithCFBBKS
import WindowFFTlog
...
def prepare_data_single(boxnumber,multipole,rmin=30,data=False):
    
    if data:
        rD,xiD,_ = np.loadtxt(opa.join(INPATH,'CFdata/dr12v4_cmass_NS_z43z75_bin5.%s.ave'%(multipole))).T[:,1:]
...
def prepare_data(boxnumber,rmin=30,data=False):
    ...
    for multipole in ['mono','quad','four']:
        result = prepare_data_single(boxnumber,multipole,rmin=rmin,data=data)
        rdata = result[0]
...
def lnlike(theta, rdata,xidata,invcovdata, free_para, fix_para,bounds,fiducial, interpolation_grid,binning=False,TableNkmu=None, window=False,dataQ=None,**kwargs):
    ...
    # Import the power spectra interpolators on the grid
        ...
        r,xi0,xi2,xi4 = getcorrelationfunctionwithCFBBKS.Ptoxi(Pkin,setkin,n=2**10,extrap=True,k_junc_low=6e-4,k_junc_high=9,ktr=4,sig=0.5,withlog=False,dampl4=False)
...
if __name__ ==  "__main__":
    ...
    gridname = "conCFAlbert"#series_cosmo.loc['gridname']
    ...
    for boxnumber in [boxnumber]:
        ...
        for indexk,rmin in enumerate(rmintab):    
            ...
            rdata,xidata,invcovdata = prepare_data(boxnumber,rmin,data=True)
    ...
    #################################    
            ...
            free_para  =  [False,False,False,True,True,True,True,True,True,True,False,False,False,False]
    ...
    #################################
        ...
        first_try = np.array([ 3.04677123,  0.30496189,  0.67660229,  1.82263321,  0.55049599,
       -0.29812687,  0.0424936 , -0.59009957, -0.11713539, -0.01373622,
        0.        ,  0.        ,  0.        ,  0.        ])
        ...
        result  =  op.minimize(chi2, all_true[free_para],method = 'SLSQP',bounds = np.array(bounds)[free_para],options = {'maxiter':100})
        ...
        np.save(opa.join(OUTPATH,"inipos%sbox_%srmin_%s")%(runtype,boxnumber,rmin),np.array(pos))
            # Start MCMC
        print("Running MCMC...")
        ...
        while loopcriteria:
            ...
            for jj in range(0, Nchains):
                    ...
                    np.save(opa.join(OUTPATH,"ChainsMidway/samplerchainmid%sbox_%srmin_%srun_%s")%(runtype,boxnumber,rmin,jj),sampler[jj].chain[:20,::10,:])
            ...
            for jj in range(0, Nchains):
                ...
                np.savetxt(opa.join(OUTPATH,'AcceptanceFr%sbox_%srmin_%srun_%s.dat'%(runtype,boxnumber,rmin,jj)),[np.mean(sampler[jj].acceptance_fraction)])
        # Estimate the integrated autocorrelation time for the time series in each
            ...
            for jj in range(Nchains):
                np.save(opa.join(OUTPATH,"samplerchain%sbox_%srmin_%srun_%s")%(runtype,boxnumber,rmin,jj),sampler[jj].chain[:,::5])
                np.save(opa.join(OUTPATH,"lnlikechain%sbox_%srmin_%srun_%s")%(runtype,boxnumber,rmin,jj),sampler[jj].lnprobability[:,::5])
            ...
            np.savetxt(opa.join(OUTPATH,"mcmcarray%sbox_%srmin_%s.txt")%(runtype,boxnumber,rmin),mcmc_array)