...
def run_zbEFT(config,dataQ):
    ...
    #Apply the window function to class PS
    Om=(cclass['omega_cdm']+cclass['omega_b'])/cclass['h']**2
    setk,Pkclass=np.loadtxt(czbEFT['PathToLinearPowerSpectrum']).T
    kPS,PS=zbetools.Classtomultipoles(setk,Pkclass,Om,cclass['z_pk'])
    PSwindow=np.concatenate(WindowFFTlog.transformQ(PS,kPS,kPS,dataQ))
    restrans=np.array(zbetools.multipolestoClass(kPS,PSwindow,Om,cclass['z_pk'])).T
    restrans=restrans[restrans[:,0]>6e-5,:]
    np.savetxt(czbEFT['PathToLinearPowerSpectrum'],restrans)
    # Run zbEFT
    command = [czbEFTw['zbEFT_exe'],czbEFTw['zbEFT_configf']]
    zbetools.runcommand(command, czbEFTw['logfile'], 
                        czbEFT['PathToFolderOutput'], cwd=czbEFTw['outpath'])

    return czbEFT['PathToFolderOutput']
