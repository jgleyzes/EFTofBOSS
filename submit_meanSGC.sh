...
for number in mean
do
qsub -l nodes=node05:ppn=4 -N b$number.$kmax.$simname2.$theoryCovness.$marg.$planck.$prior -o log/output_b$number.$kmax.$kmaxbisp.$simname2.Planck$planck.Prior$prior.log -e log/error_b$number.$kmax.$kmaxbisp.$simname2.Planck$planck.Prior$prior.log -v aa=$number,b=$kmax,c=$simname2,d=$planck,e=$grid,f=$kmaxbisp,g=$marg,h=$theoryCovness,ii=$prior cluster_MCMC_Mercury.sh 
done
done
done
done
