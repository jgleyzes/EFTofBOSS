	...
	if (!triangle.is_open()) {
		...
		while(triangle >> k1 >> k2 >> k3) {
			...
			for (unsigned int i = 0 ; i < 11 ; i++) { // 11 integrations to perform
				...
				write << setw(pre) << 1./2.*res ; // (2l+1)/2 * Integral ;
			}
			
			// One of the integrand does not depend on mu and phi so we just multiply it by 4*PI to get the result
			write << setw(pre) << 2.*M_PI* 1./(pow(apar,2)*pow(aperp,4)) * pow(nbar,-2) << endl ;
		} ;
	}
...
static int IntegrandBispectrumAP_CUBA (const int *ndim, const double a[], const int *ncomp, double ff[], void *params) {
	...
	ff[0] = 2.*2.*M_PI * IntegrandBispectrumAP (p.id, p.k1, p.k2, p.k3, mu, phi, p.p11, p.nbar, p.aperp, p.apar) ;
}
